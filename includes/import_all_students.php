...
try {
    ...
    if ($fileExtension === 'csv') {
        ...
        if (($handle = fopen($fileTmpName, 'r')) !== false) {
            ...
            // Expected columns: Student Number, Email, First Name, Last Name (required), Middle Initial, Suffix, Program (optional)
    ...
    foreach ($students as $student) {
        ...
        if ($existingStudent) {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO students (student_number, student_email, first_name, last_name, middle_initial, suffix, program, created_at, teacher_email)
                VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), ?)
            ");
            $stmt->execute([
