...
<body>
    ...
    <script>
        ...
        document.getElementById('btn-request').addEventListener('click', async () => {
            ...
            showLoader('Sending code...', 'Please wait while we send the verification code.');
            try {
                const resp = await postReset({ action: 'request_code', email });
                const isCooldown = !!resp.cooldown;
                const statusType = resp.success || isCooldown ? 'success' : 'error';
                const message = isCooldown
                    ? 'A verification code was already sent recently. You can verify it now.'
                    : (resp.message || 'Request failed');
                showMessage(statusType, message);
                if (resp.success || resp.cooldown) {
        ...
        document.getElementById('btn-verify').addEventListener('click', async () => {
            ...
            showLoader('Verifying code...', 'Please wait while we verify your code.');
            try {
        ...
        document.getElementById('btn-reset').addEventListener('click', async () => {
            ...
            if (resp.success) {
                Swal.fire({
                    ...
                    text: resp.message || 'Password updated. Redirecting to login...',
                    timer: 2000,
